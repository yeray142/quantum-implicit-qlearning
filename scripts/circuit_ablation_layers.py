#!/usr/bin/env python3
"""
Circuit Design Ablation — DRU Reuploading Layers Sweep
=======================================================
Sweeps n_layers ∈ {1, 2, 3} with n_qubits adjusted to satisfy
L ≤ floor(log2(n_qubits)):
  n_layers=1  →  n_qubits=4
  n_layers=2  →  n_qubits=4  (base config ★)
  n_layers=3  →  n_qubits=8

Related scripts:
  topology / measurement / datasize  → circuit_ablation_main.py
  qubit×layer joint grid             → circuit_ablation_qubit_layer.py

Usage
-----
  python experiments/circuit_ablation_layers.py
  python experiments/circuit_ablation_layers.py --seeds 0 1 2 3 4 --wandb-offline
  python experiments/circuit_ablation_layers.py --steps 50000 --dry-run
"""
...
def ablation_layers(seeds: List[int], env_cfg: dict, n_steps: int,
                    wandb_offline: bool) -> None:
    print("\n" + "=" * 65)
    print("  ABLATION 1: DRU Reuploading Layers")
    print("=" * 65)
    buffer = load_minari_dataset(env_cfg["dataset_id"], device="cpu")
    all_results = {}

    for cond_key, kwargs in LAYERS_CONDITIONS.items():
        print(f"\n  === {cond_key} ===")
        assert kwargs["n_layers"] <= math.floor(math.log2(kwargs["n_qubits"]))
        seed_results = []
        for seed in seeds:
            print(f"  seed={seed}", end="  ")
            r = run_condition(
                cond_key, seed, buffer, env_cfg,
                use_flex=False, trainer_kwargs=kwargs,
                n_steps=n_steps,
                wandb_tags=["ablation:layers"],
                wandb_offline=wandb_offline,
            )
            seed_results.append(r)
            print(f"loss={r['loss'][-1]:.4f}  "
                  f"adv={np.mean(r['adv_mean'][-50:]):.3f}  "
                  f"ms={np.mean(r['ms_per_step']):.1f}")
        all_results[cond_key] = seed_results

    _print_summary("LAYERS", all_results)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Quantum-IQL Circuit Ablation — DRU Layers Sweep",
...
def main() -> None:
    ...
    print(f"\n{'='*65}")
    print(f"  Quantum-IQL Circuit Ablation — DRU Layers Sweep")
    print(f"  Hardware : {hw}")
    print(f"  Env      : {args.env}  ({env_cfg['dataset_id']})")
    print(f"  Conditions: {list(LAYERS_CONDITIONS.keys())}")
    ...
    try:
        ablation_layers(args.seeds, env_cfg, args.steps, args.wandb_offline)
