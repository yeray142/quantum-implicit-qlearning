...
# FIX 1: PennyLane's default.qubit does NOT run on CUDA even with backprop.
# Quantum circuit parameters must always stay on CPU.
# We keep TORCH_DEVICE for classical scalars (a, b) and inputs, but the
# circuit call itself uses .cpu() tensors (see QuantumValueNet.forward).
if torch.cuda.is_available():
    DIFF_METHOD   = "backprop"
    QUANTUM_BATCH = 256
    print("[quantum] GPU available — using diff_method='backprop' + default.qubit (full batch=256)")
    print("[quantum] NOTE: quantum circuit params kept on CPU; only classical scalars use GPU.")
else:
    DIFF_METHOD   = "adjoint"
    QUANTUM_BATCH = 4
...
class QuantumValueNet(nn.Module):
    def __init__(
        ...
        # FIX 1 (continued): theta and w are always kept on CPU because
        # PennyLane's default.qubit simulator cannot accept CUDA tensors.
        # We register them as parameters but override .to() via the device
        # management in forward(). a and b are classical scalars that CAN
        # live on the GPU for the final affine step.
    ...
    def forward(self, s: torch.Tensor) -> torch.Tensor:
        # FIX 1: Always pass CPU tensors into the PennyLane circuit.
        # s may be on CUDA (created with device=TORCH_DEVICE in train()),
        # so we move xs, theta, and w to CPU before the circuit call,
        # then move the expectation values back to s.device for the loss.
        xs_cpu    = torch.arctan(s[:, :self.n_qubits]).cpu()
        theta_cpu = self.theta.cpu()
        w_cpu     = self.w.cpu()

        ev = torch.stack([self._circuit(theta_cpu, w_cpu, x) for x in xs_cpu])

        # Move result back to the original device before the affine step
        ev = ev.to(s.device)
...
def train(net: QuantumValueNet):
    # FIX 2: Added startup print so you immediately see progress,
    # rather than waiting silently until step 10 000.
    print(f"    Starting training: {N_STEPS} steps, batch={BATCH}, device={TORCH_DEVICE}")
...
# ─────────────────────────────────────────────────────────────────────────────
# 10. Checkpoint helper — serialises tuple keys for JSON
# FIX 3: Removed the duplicate first definition of checkpoint() that lacked
# tuple-key serialisation. Only this correct version is kept.
# ─────────────────────────────────────────────────────────────────────────────
