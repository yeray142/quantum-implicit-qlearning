...
Classical (69\,121 params)
  & 3505.8 & 16.2  & 0.5\%  & 69\,121 & $-0.96$ & 0.70--0.88 \\
...
Quantum Fixed-Warmup (Fix A only, 146 params)
  ...
  & $+1.33$ & 0.36--0.86 \\
\textcolor{darkgreen}{Quantum Fixed-C (Fix A\,+\,B\,+\,C, 146 params)}
  & \textcolor{darkgreen}{3221.5}
  & \textcolor{darkgreen}{440.1}
  & \textcolor{darkgreen}{13.7\%}
  & \textcolor{darkgreen}{146}
  & \textcolor{darkgreen}{$-2.36$}
  & \textcolor{darkgreen}{0.44--0.88} \\
\midrule
\end{tabular}
\end{table}
...
\textbf{Config} & \textbf{Mean return} & \textbf{CV} &
  ...
  $N(\|\nabla\theta\|{>}10)$ \\
\midrule
Classical ($n=3$)             & $3505.8 \pm 16.2$  &  0.5\% & 3/3 & 2/3 & 0/3 \\
...
\textcolor{darkgreen}{Quantum Fixed-C ($n=8$)}
                               & \textcolor{darkgreen}{$3221.5 \pm 440.1$}
                               & \textcolor{darkgreen}{13.7\%}
                               & \textcolor{darkgreen}{7/8}
...
Fix~C achieves the highest mean return in the Fix-A family ($3221.5$, lowest CV
13.7\%) and is the only configuration with zero gradient explosions. Five of
eight Fixed-C seeds reach the on-target band, the best rate in the Fix-A family.
...
The Fix-A pool vs Classical comparison is the primary result: $n=24$ quantum
seeds vs $n=3$ classical, $p=0.0075$, $d=-0.87$ (large effect). Fixed-C alone
is not significant at $n=8$ vs $n=3$ ($p=0.131$, $d=-0.91$) due to the classical
power problem. The all-quantum comparison ($p=0.0007$) spans all 35 quantum runs
and confirms aggregate quantum underperformance, but is not informative about
...
\midrule
Quantum Fixed-C ($n=8$) vs Classical ($n=3$)
           & 7/8 vs 3/3 & $[44\%,97\%]$ vs $[41\%,99\%]$
           & $p=1.000$ \\
\midrule
Quantum Fixed-C ($n=8$) vs Quantum No-Warmup ($n=8$)
           & 7/8 vs 2/8 & $[44\%,97\%]$ vs $[7\%,59\%]$
           & $p=0.119$ \\
\midrule
Quantum Fixed ($n=8$) vs Quantum No-Warmup ($n=8$)
           & 6/8 vs 2/8 & $[44\%,91\%]$ vs $[7\%,59\%]$
           & $p=0.314$ \\
\midrule
Fix-A pool ($n=24$) vs Default pool (quantum + quantum-no-warmup, $n=11$)
           ...
           & $p=\mathbf{0.0004}$ \\[2pt]
\midrule
All quantum on-target ($n=35$) vs Classical ($n=3$)
           & 17/35 vs 2/3 & $[28\%,60\%]$ vs $[41\%,99\%]$
           & $p=0.041$ \\
\bottomrule
\end{tabular}
\end{center}

The Fix-A pool vs default pool comparison is significant at $p=0.0004$ (Fisher),
establishing that Fix-A improves direction-correct calibration relative to
default initialisation. The all-quantum on-target vs classical on-target
($p=0.041$) confirms the aggregate calibration deficit. The classical pool at
$n=3$ is too small for the Fisher test to reach significance against the Fix-A
pool ($p=0.280$), but the point estimates (20/24 vs 3/3) show a clear pattern.
...
\midrule
Classical direction-correct ($n=3$)         & 3/3 = 100\%    & $[41\%,99\%]$ \\
...
The Wilson CIs show that the classical direction-correct rate (100\%) has a
wide CI ($[41\%,99\%]$) due to $n=3$. The Fix-A pool (83.3\%, CI $[57\%,91\%]$)
shows substantial improvement over the default pool (18.2\%, CI $[4\%,41\%]$),
with non-overlapping CIs indicating a reliable difference. Fixed-C achieves the
highest on-target rate at 75\% (6/8), followed by Fixed and Fixed-Warmup at 62.5\% (5/8).
...
\textbf{(2) The Fix-A pool ($n=24$) is significantly worse than classical on
return ($p=0.0075$, $d=-0.87$).}
Fix-A pool mean return $3152\pm577$ vs classical $3505.8\pm16.2$. This is the
only conventionally significant return comparison. The quantum-classical gap is
large and reliable at the pool level, even though pairwise comparisons at
$n=8$ are underpowered.
